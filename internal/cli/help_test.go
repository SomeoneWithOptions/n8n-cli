package cli

import (
	"strings"
	"testing"

	"github.com/spf13/cobra"
	"github.com/spf13/pflag"
)

// TestHelpIsDescriptive enforces the PLAN.md help and discoverability
// contract: a human or AI agent must be able to act from --help alone.
// Every command needs Short, Long, and Example; every flag needs a usage
// string; --help must succeed at every level.
func TestHelpIsDescriptive(t *testing.T) {
	root := NewRootCommand(Options{Streams: Streams{In: strings.NewReader(""), Out: nil, Err: nil}, Version: testVersion})

	var visit func(path string, c *cobra.Command)
	visit = func(path string, c *cobra.Command) {
		path = strings.TrimSpace(path + " " + c.Name())
		if c.Name() == "" {
			path = "n8n"
		}
		if strings.TrimSpace(c.Short) == "" {
			t.Errorf("%s: Short is empty", path)
		}
		if len(strings.TrimSpace(c.Long)) < 40 {
			t.Errorf("%s: Long too short (%d chars), want >= 40 describing workflow and next steps", path, len(strings.TrimSpace(c.Long)))
		}
		if strings.TrimSpace(c.Example) == "" {
			t.Errorf("%s: Example is empty; add copy-pasteable invocations", path)
		}
		c.Flags().VisitAll(func(f *pflag.Flag) {
			if strings.TrimSpace(f.Usage) == "" {
				t.Errorf("%s --%s: flag usage is empty", path, f.Name)
			}
		})
		c.PersistentFlags().VisitAll(func(f *pflag.Flag) {
			if strings.TrimSpace(f.Usage) == "" {
				t.Errorf("%s --%s: flag usage is empty", path, f.Name)
			}
		})
		for _, sub := range c.Commands() {
			visit(path, sub)
		}
	}
	for _, sub := range root.Commands() {
		visit("n8n", sub)
	}
	// Root itself.
	if strings.TrimSpace(root.Short) == "" {
		t.Error("n8n: Short is empty")
	}
	if len(strings.TrimSpace(root.Long)) < 40 {
		t.Error("n8n: Long too short")
	}
	if strings.TrimSpace(root.Example) == "" {
		t.Error("n8n: Example is empty")
	}
}

func TestHelpSucceedsAtEveryLevel(t *testing.T) {
	paths := [][]string{
		{},
		{"auth"},
		{"auth", "login"},
		{"auth", "status"},
		{"auth", "logout"},
		{"discover"},
		{"audit"},
		{"audit", "generate"},
		{"community-package"},
		{"community-package", "list"},
		{"community-package", "install"},
		{"community-package", "update"},
		{"community-package", "uninstall"},
		{"tag"},
		{"tag", "list"},
		{"tag", "create"},
		{"tag", "get"},
		{"tag", "update"},
		{"tag", "delete"},
		{"variable"},
		{"variable", "list"},
		{"variable", "create"},
		{"variable", "update"},
		{"variable", "delete"},
		{"role"},
		{"role", "list"},
		{"role", "create"},
		{"role", "get"},
		{"role", "update"},
		{"role", "delete"},
		{"role-mapping"},
		{"role-mapping", "list"},
		{"role-mapping", "create"},
		{"role-mapping", "move"},
		{"role-mapping", "update"},
		{"role-mapping", "delete"},
		{"user"},
		{"user", "list"},
		{"user", "create"},
		{"user", "get"},
		{"user", "role"},
		{"user", "role", "set"},
		{"user", "delete"},
		{"project"},
		{"project", "list"},
		{"project", "create"},
		{"project", "update"},
		{"project", "delete"},
		{"project", "user"},
		{"project", "user", "list"},
		{"project", "user", "add"},
		{"project", "user", "role"},
		{"project", "user", "role", "set"},
		{"project", "user", "remove"},
		{"folder"},
		{"folder", "list"},
		{"folder", "create"},
		{"folder", "get"},
		{"folder", "update"},
		{"folder", "delete"},
		{"data-table"},
		{"data-table", "list"},
		{"data-table", "create"},
		{"data-table", "get"},
		{"data-table", "update"},
		{"data-table", "delete"},
		{"data-table", "row"},
		{"data-table", "row", "list"},
		{"data-table", "row", "insert"},
		{"data-table", "row", "update"},
		{"data-table", "row", "upsert"},
		{"data-table", "row", "clear"},
		{"data-table", "row", "delete"},
		{"data-table", "column"},
		{"data-table", "column", "list"},
		{"data-table", "column", "create"},
		{"data-table", "column", "update"},
		{"data-table", "column", "delete"},
		{"workflow"},
		{"workflow", "list"},
		{"workflow", "get"},
		{"workflow", "diff"},
		{"workflow", "create"},
		{"workflow", "update"},
		{"workflow", "delete"},
		{"workflow", "archive"},
		{"workflow", "unarchive"},
		{"workflow", "publish"},
		{"workflow", "unpublish"},
		{"workflow", "transfer"},
		{"workflow", "history"},
		{"workflow", "version"},
		{"workflow", "version", "get"},
		{"workflow", "tag"},
		{"workflow", "tag", "list"},
		{"workflow", "tag", "set"},
		{"workflow", "activate"},
		{"workflow", "deactivate"},
		{"workflow", "version", "get-legacy"},
		{"execution"},
		{"execution", "list"},
		{"execution", "get"},
		{"execution", "delete"},
		{"execution", "stop-many"},
		{"execution", "stop"},
		{"execution", "retry"},
		{"execution", "watch"},
		{"execution", "trace"},
		{"execution", "tag"},
		{"execution", "tag", "list"},
		{"execution", "tag", "set"},
		{"evaluation"},
		{"evaluation", "list"},
		{"evaluation", "create"},
		{"evaluation", "get"},
		{"evaluation", "cancel"},
		{"evaluation", "case"},
		{"evaluation", "case", "list"},
		{"insight"},
		{"insight", "summary"},
		{"security-policy"},
		{"security-policy", "get"},
		{"security-policy", "set"},
		{"otel"},
		{"otel", "get"},
		{"otel", "set"},
		{"otel", "test-trace"},
		{"log-stream"},
		{"log-stream", "event-type"},
		{"log-stream", "event-type", "list"},
		{"log-stream", "destination"},
		{"log-stream", "destination", "list"},
		{"log-stream", "destination", "create"},
		{"log-stream", "destination", "get"},
		{"log-stream", "destination", "update"},
		{"log-stream", "destination", "test"},
		{"log-stream", "destination", "delete"},
		{"ldap"},
		{"ldap", "get"},
		{"ldap", "set"},
		{"ldap", "sync"},
		{"ldap", "sync", "history"},
		{"ldap", "sync", "run"},
		{"oidc"},
		{"oidc", "get"},
		{"oidc", "set"},
		{"saml"},
		{"saml", "get"},
		{"saml", "set"},
		{"source-control"},
		{"source-control", "status"},
		{"source-control", "push"},
		{"source-control", "pull"},
		{"package"},
		{"package", "export"},
		{"package", "import"},
		{"git-connection"},
		{"git-connection", "list"},
		{"git-connection", "create"},
		{"git-connection", "get"},
		{"git-connection", "update"},
		{"git-connection", "delete"},
		{"git-connection", "clone"},
		{"git-connection", "disconnect"},
		{"git-connection", "project"},
		{"git-connection", "project", "list"},
		{"git-connection", "project", "add"},
		{"git-connection", "project", "remove"},
		{"git-connection", "push"},
		{"git-connection", "pull"},
		{"promotion"},
		{"promotion", "provider"},
		{"promotion", "provider", "list"},
		{"promotion", "provider", "create"},
		{"promotion", "provider", "get"},
		{"promotion", "provider", "update"},
		{"promotion", "provider", "delete"},
		{"promotion", "connection"},
		{"promotion", "connection", "list"},
		{"promotion", "connection", "create"},
		{"promotion", "connection", "get"},
		{"promotion", "connection", "update"},
		{"promotion", "connection", "delete"},
		{"promotion", "config"},
		{"promotion", "config", "apply"},
		{"promotion", "config", "apply", "set"},
		{"promotion", "config", "promote"},
		{"promotion", "config", "promote", "set"},
		{"promotion", "config", "delete"},
		{"promotion", "checkout"},
		{"promotion", "checkout", "clone"},
		{"promotion", "checkout", "disconnect"},
		{"promotion", "project"},
		{"promotion", "project", "list"},
		{"promotion", "project", "add"},
		{"promotion", "project", "remove"},
		{"promotion", "promote"},
		{"promotion", "apply"},
		{"config"},
		{"config", "context"},
		{"config", "context", "list"},
		{"config", "context", "use"},
		{"config", "context", "delete"},
		{"version"},
		{"update"},
		{"completion"},
	}
	for _, p := range paths {
		args := append(append([]string{}, p...), "--help")
		got := run(t, args...)
		name := "n8n " + strings.Join(p, " ")
		if got.code != ExitSuccess {
			t.Errorf("%s --help: exit code = %d, want %d (stderr: %s)", name, got.code, ExitSuccess, got.stderr)
		}
		for _, want := range []string{"Usage:", "Examples:", "--help"} {
			if !strings.Contains(got.stdout, want) {
				t.Errorf("%s --help: stdout missing %q\n--- stdout ---\n%s", name, want, got.stdout)
			}
		}
	}
}
